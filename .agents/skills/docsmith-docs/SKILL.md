...
This is a **Composer package** (`mrpunyapal/laravel-extended-validation`), not an application. Document how a consumer installs it and the validation rules. Every rule works via class instance, fluent Rule macro, and string syntax.

Run tests with `composer test`. Rebuild docs with `composer run docs:build`.
