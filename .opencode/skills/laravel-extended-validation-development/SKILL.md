...
| Semver | `Semver` | `Rule::semver(?string $prefix = null)` | `semver`, `semver:v`, `semver:optional` | `1.0.0`, `v1.2.3` |
| Base64 | `Base64String` | `Rule::base64String(...$mimes, bool $urlSafe = false)` | `base64_string`, `base64_string:url_safe` | Base64 strings, Data URIs, URL-safe base64 |
...
| Country Code | `CountryCode` | `Rule::countryCode(string $format = 'alpha2')` | `country_code`, `country_code:alpha3`, `country_code:any` | ISO 3166-1 alpha-2, alpha-3, or either |
| Hex Color | `HexColor` | `Rule::hexColor(?string $prefix = null)` | `hex_color` | `#fff`, `4f46e5` (withoutHash), optional hash |
...
| Email Domain | `EmailDomain` | `Rule::emailDomain($allow, $block)` | `email_domain` | Allowed/blocked domains and wildcards (*.domain.com) |
| Not Hashed | `NotHashed` | `Rule::notHashed()` | `not_hashed` | Rejects bcrypt/argon hashes |
| Alpha Underscore | `AlphaUnderscore` | `Rule::alphaUnderscore(bool $ascii = false)` | `alpha_underscore`, `alpha_underscore:ascii` | `user_123` (letters, numbers, _) |
| Unless Between | `UnlessBetween` | `Rule::unlessBetween($min, $max)` | `unless_between:min,max` | Outside numeric range (auto-normalizes bounds) |
...
| URL Protocol | `UrlProtocol` | `Rule::urlProtocol(...$p)` | `url_protocol:https,sftp` | Allowed protocol schemes (with or without `://`) |
| Snake Case | `SnakeCase` | `Rule::snakeCase(?bool $capital = false)` | `snake_case`, `snake_case:capital` | `user_name_123`, `API_KEY_SECRET` |
| Multiple Of | `MultipleOf` | `Rule::multipleOf($step)` | `multiple_of:step` | Exact multiple of step |
